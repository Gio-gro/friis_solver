...
with col_sys1:
    st.number_input(
        ...
        on_change=react,
        kwargs={"symbol": freq, "key": "freq"},
    )
with col_sys2:
    st.number_input(
        "Wavelength (m)", key="wvlen", format=num_format, disabled=is_disabled("wvlen")
    )
...
with col1:
    ...
    st.number_input(
        "Tx Power, $P_t$ (W)", key="p_t", format=num_format, disabled=is_disabled("p_t")
    )
    st.number_input(
        "Tx Gain, $G_t$ (ratio)",
        key="g_t",
    ...
    st.number_input(
        "Tx Loss Factor, $f_t$ (ratio)",
        key="f_t",
        format=num_format,
        disabled=is_disabled("f_t"),
    )
    st.number_input(
        "EIRP (W)", key="eirp", format=num_format, disabled=is_disabled("eirp")
    )
...
with col3:
    ...
    st.number_input(
        "Rx Power, $P_r$ (W)", key="p_r", format=num_format, disabled=is_disabled("p_r")
    )
    st.number_input(
        "Rx Gain, $G_r$ (ratio)",
        key="g_r",
